    ...
    <el-col :span="24">
      <el-card>
        <div slot="header" class="tw-flex tw-justify-end tw-items-center">
          <router-link v-slot="{ href, navigate }" v-permission="['create']" :to="{name: 'MemberCreate'}" custom>
        ...
        <el-row :gutter="20">
          <el-col :span="24" class="table-responsive">
            ...
            >
              <el-table-column align="center" sortable="custom" prop="id" :label="$t('table.member.id')" width="70px">
                <template slot-scope="{ row }">
                  {{ row.id }}
                </template>
              </el-table-column>
              <el-table-column data-generator="code" prop="code" :label="$t('table.member.code')" align="left" header-align="center">
                <template slot-scope="{ row }">
                  {{ row.code }}
                </template>
              </el-table-column>
              <el-table-column data-generator="name" prop="name" :label="$t('table.member.name')" align="left" header-align="center">
                <template slot-scope="{ row }">
                  {{ row.name }}
                </template>
              </el-table-column>
              <el-table-column data-generator="sns_link" prop="sns_link" :label="$t('table.member.sns_link')" align="left" header-align="center">
                <template slot-scope="{ row }">
                  {{ row.sns_link }}
                </template>
              </el-table-column>
              <el-table-column data-generator="is_block" prop="is_block" sortable="custom" :label="$t('table.member.is_block')" align="center" header-align="center">
                <template slot-scope="{ row }">
            ...
            <!--{{$TEMPLATES_NOT_DELETE_THIS_LINE$}}-->
              ...
              <el-table-column :label="$t('table.actions')" align="center" class-name="small-padding fixed-width">
                <template slot-scope="{ row }">
                  <router-link v-permission="['edit']" :to="{name: 'MemberEdit', params: {id: row.id}}"><i class="el-icon-edit el-link el-link--primary tw-mr-2" /></router-link>
...
<script>
  ...
  import MembersResource from '@/api/v1/member';
  const memberResource = new MembersResource();
  ...
  export default {
    ...
    methods: {
      ...
      remove(id) {
        this.$confirm(this.$t('messages.delete_confirm', { attribute: this.$t('table.member.id') + '#' + id }), this.$t('messages.warning'), {
