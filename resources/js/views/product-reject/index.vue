    ...
    <el-col :span="24">
      <el-card>
        <div slot="header" class="tw-flex tw-justify-end tw-items-center">
          <router-link v-slot="{ href, navigate }" v-permission="['create']" :to="{name: 'ProductRejectCreate'}" custom>
        ...
        <el-row :gutter="20">
          <el-col :span="24" class="table-responsive">
            ...
            >
              <el-table-column align="center" sortable="custom" prop="id" :label="$t('table.product_reject.id')" width="70px">
                <template slot-scope="{ row }">
                  {{ row.id }}
                </template>
              </el-table-column>
              <el-table-column data-generator="total" prop="total" :label="$t('table.product_reject.total')" align="center" header-align="center">
                <template slot-scope="{ row }">
                  {{ row.total }}
                </template>
              </el-table-column>
              <el-table-column data-generator="price" prop="price" :label="$t('table.product_reject.price')" align="center" header-align="center">
                <template slot-scope="{ row }">
                  {{ row.price }}
                </template>
              </el-table-column>
              <el-table-column data-generator="note" prop="note" :label="$t('table.product_reject.note')" align="left" header-align="center">
                <template slot-scope="{ row }">
                  {{ row.note }}
                </template>
              </el-table-column>
            ...
            <!--{{$TEMPLATES_NOT_DELETE_THIS_LINE$}}-->
              ...
              <el-table-column :label="$t('table.actions')" align="center" class-name="small-padding fixed-width">
                <template slot-scope="{ row }">
                  <router-link v-permission="['edit']" :to="{name: 'ProductRejectEdit', params: {id: row.id}}"><i class="el-icon-edit el-link el-link--primary tw-mr-2" /></router-link>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      remove(id) {
        this.$confirm(this.$t('messages.delete_confirm', { attribute: this.$t('table.product_reject.id') + '#' + id }), this.$t('messages.warning'), {
