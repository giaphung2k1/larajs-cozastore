    ...
    <el-col :span="24">
      <el-card>
        <div slot="header" class="tw-flex tw-justify-end tw-items-center">
          <router-link v-slot="{ href, navigate }" v-permission="['create']" :to="{name: 'SizeCreate'}" custom>
        ...
        <el-row :gutter="20">
          <el-col :span="24" class="table-responsive">
            ...
            >
              <el-table-column align="center" sortable="custom" prop="id" :label="$t('table.size.id')" width="70px">
                <template slot-scope="{ row }">
                  {{ row.id }}
                </template>
              </el-table-column>
              <el-table-column data-generator="name" prop="name" :label="$t('table.size.name')" align="left" header-align="center">
                <template slot-scope="{ row }">
                  {{ row.name }}
                </template>
              </el-table-column>
              ...
              <el-table-column :label="$t('table.actions')" align="center" class-name="small-padding fixed-width">
                <template slot-scope="{ row }">
                  <router-link v-permission="['edit']" :to="{name: 'SizeEdit', params: {id: row.id}}"><i class="el-icon-edit el-link el-link--primary tw-mr-2" /></router-link>
...
<script>
  ...
  import SizesResource from '@/api/v1/size';
  const sizeResource = new SizesResource();
    ...
    methods: {
      async getList() {
        try {
          ...
          const { data } = await sizeResource.list(this.table.listQuery);
      ...
      remove(id) {
        this.$confirm(this.$t('messages.delete_confirm', { attribute: this.$t('table.size.id') + '#' + id }), this.$t('messages.warning'), {
