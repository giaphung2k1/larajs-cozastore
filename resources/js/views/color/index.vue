    ...
    <el-col :span="24">
      <el-card>
        <div slot="header" class="tw-flex tw-justify-end tw-items-center">
          <router-link v-slot="{ href, navigate }" v-permission="['create']" :to="{name: 'ColorCreate'}" custom>
        ...
        <el-row :gutter="20">
          <el-col :span="24" class="table-responsive">
            ...
            >
              <el-table-column align="center" sortable="custom" prop="id" :label="$t('table.color.id')" width="70px">
                <template slot-scope="{ row }">
                  {{ row.id }}
                </template>
              </el-table-column>
              <el-table-column data-generator="name" prop="name" :label="$t('table.color.name')" align="left" header-align="center">
                <template slot-scope="{ row }">
                  {{ row.name }}
                </template>
              </el-table-column>
            ...
            <!--{{$TEMPLATES_NOT_DELETE_THIS_LINE$}}-->
              ...
              <el-table-column :label="$t('table.actions')" align="center" class-name="small-padding fixed-width">
                <template slot-scope="{ row }">
                  <router-link v-permission="['edit']" :to="{name: 'ColorEdit', params: {id: row.id}}"><i class="el-icon-edit el-link el-link--primary tw-mr-2" /></router-link>
...
<script>
  ...
  import ColorsResource from '@/api/v1/color';
  const colorResource = new ColorsResource();
    ...
    methods: {
      async getList() {
        try {
          ...
          const { data } = await colorResource.list(this.table.listQuery);
      ...
      remove(id) {
        this.$confirm(this.$t('messages.delete_confirm', { attribute: this.$t('table.color.id') + '#' + id }), this.$t('messages.warning'), {
          ...
          try {
            this.table.loading = true;
            await colorResource.destroy(id);
